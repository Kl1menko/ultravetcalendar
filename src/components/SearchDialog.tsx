  ...
  return (
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="flex max-h-[80svh] max-w-[420px] flex-col gap-0 overflow-hidden rounded-[18px] border-white/80 p-0 shadow-[0_24px_80px_rgba(17,24,39,0.22)] md:max-h-[78dvh] md:max-w-[720px] md:rounded-[28px]">
        <DialogHeader className="border-b border-[var(--line)] bg-white px-4 pt-4 pb-3 text-left md:px-6 md:pt-5 md:pb-4">
            ...
            >
              {results.map((item) => (
                  <motion.button
                    ...
                    className="matte-appt-card grid min-h-16 w-full grid-cols-[44px_1fr] gap-2 rounded-lg p-2.5 text-left transition-all active:scale-[0.985] md:rounded-2xl md:p-3 md:hover:-translate-y-0.5 md:hover:border-[var(--lg-border-strong)] md:hover:shadow-[var(--desktop-soft)]"
                  >
                    <span className="text-sm font-bold text-[var(--ink)]">{item.start}</span>
